  ...
  "attachments": [
    {
      ...
      "dest_filename": "ref-friendly-name.jpg"
    }
  ],
  "source_material": {
    "name": "자료명",
...
> **필수 체크**: vault JSON을 write하기 전에 원본 사용자 메시지에서 `[Photo: ...]`, `[Video: ...]`, `[Document: ...]` 패턴을 먼저 추출한다. 하나라도 있으면 반드시 `attachments` 배열에 포함해야 한다. betty-video로 Gemini 분석을 마친 후에도 `[Video:]` 경로는 여전히 `attachments`에 포함해야 한다. `attachments`를 빠뜨리면 로컬 vault에 파일이 저장되지 않는다.

사용자 메시지에 `[Photo: <경로>]`, `[Video: <경로>]`, `[Document: <경로>]` 패턴이 있고 노트 저장 요청인 경우, JSON의 `attachments` 배열에 해당 파일 정보를 포함한다.

### 필드 규칙

- `source_path`: 컨테이너 내 경로 그대로 (`/workspace/media/photo_xxx.jpg`) — **필수**
- `filename`: source_path의 basename (`photo_xxx.jpg`) — **필수**. 반드시 `source_path`와 일치하는 실제 파일명을 사용한다
- `dest_filename`: 로컬 attachments/ 저장 시 사용할 친화적 파일명 (선택). 있으면 이 이름으로 저장되고 `![[dest_filename]]`으로 embed된다. 없으면 `filename` 사용

### 잘못된 필드명 금지

```json
// 금지: vault_path 필드는 존재하지 않음 — vault-watcher가 filename을 읽지 못해 이미지 스킵
{ "source_path": "/workspace/media/photo_xxx.jpg", "vault_path": "attachments/ref.jpg" }

// 금지: filepath, path 등 비표준 필드명 사용
{ "filepath": "/workspace/media/photo_xxx.jpg" }
```

```json
// 올바른 예시 (dest_filename 있음)
{
  "source_path": "/workspace/media/photo_AQADTQ1rG9kCCVZ-.jpg",
  "filename": "photo_AQADTQ1rG9kCCVZ-.jpg",
  "dest_filename": "ref-glassmorphism-sphere-grid.jpg"
}

// 올바른 예시 (dest_filename 없음 — filename 그대로 사용)
{
  "source_path": "/workspace/media/photo_AQADTQ1rG9kCCVZ-.jpg",
  "filename": "photo_AQADTQ1rG9kCCVZ-.jpg"
}

// 올바른 예시 (영상 — [Video:] 패턴도 동일하게 attachments에 포함)
{
  "source_path": "/workspace/media/animation_AgADCR0AAhceEFY.mp4",
  "filename": "animation_AgADCR0AAhceEFY.mp4",
  "dest_filename": "ref-motion-review.mp4"
}
```

### 복수 이미지 전체 포함 규칙

메시지에 여러 이미지가 포함된 경우, **모든 이미지**를 `attachments` 배열에 포함한다. 1장만 포함하거나 일부를 생략하면 vault-watcher가 나머지 이미지를 복사하지 못한다.

메시지에 `[Photo: ...]`/`[Video: ...]` 태그가 N개 있으면 `attachments` 배열도 반드시 N개여야 한다.
JSON을 생성하기 전에 메시지 내 `[Photo:]`/`[Video:]` 태그 수를 세고, 그 수와 attachments 배열
길이가 일치하는지 확인한다. 불일치 시 누락된 태그의 경로를 찾아 추가한다.

```json
// 올바른 예시: 4장 모두 포함
"attachments": [
  { "source_path": "/workspace/media/photo_A.jpg", "filename": "photo_A.jpg" },
  { "source_path": "/workspace/media/photo_B.jpg", "filename": "photo_B.jpg" },
  { "source_path": "/workspace/media/photo_C.jpg", "filename": "photo_C.jpg" },
  { "source_path": "/workspace/media/photo_D.jpg", "filename": "photo_D.jpg" }
]
```

### URL 추출 미디어 첨부

agent-browser로 URL에서 추출한 이미지 및 영상은 `/workspace/media/<filename>` 경로에 저장한다.
attachments 항목의 source_path는 `/workspace/media/<filename>` 형식을 사용한다.
기존 `/workspace/media/` 파일을 덮어쓰거나 삭제하지 않는다.
...
0. **[필수] 첨부 파일 추출** — JSON 작성 전 반드시 실행

   원본 메시지에서 `[Photo: ...]`, `[Video: ...]`, `[Document: ...]` 태그를 모두 찾아 경로를 기록한다.
   태그가 하나라도 있으면 JSON의 `attachments` 배열에 포함한다.

   - `[Video: /workspace/media/animation_AgAD....mp4]` → `attachments`에 포함 (betty-video 분석 완료 후에도 반드시 포함)
   - `[Photo: /workspace/media/photo_xxx.jpg]` → `attachments`에 포함
   - 태그 없음 → `attachments` 생략 또는 `[]`
...
**attachments 필드**: 기존 노트에 이미지/파일을 함께 추가할 때 사용한다. create action의 attachments와 동일한 스키마이며, 생략하거나 빈 배열 `[]`를 사용하면 첨부 없이 content만 추가된다.

> **⚠️ content 내 frontmatter 금지**: `content` 필드에 `---`로 감싼 frontmatter를 포함하지 마라. vault-watcher가 append 시 frontmatter를 본문에 그대로 삽입하여 노트가 깨진다.
>
> - 금지: `"content": "---\ntags: [foo]\n---\n추가 내용"`
> - 허용: `"content": "추가 내용"`

> **⚠️ content 내 `![[...]]` 삽입 금지**: attachments를 사용하는 경우, `content`에 `![[filename]]`을 직접 넣지 마라. vault-watcher가 첨부 파일 복사 후 자동으로 embed를 삽입한다.

### 웹 검색 이미지를 기존 노트에 추가하는 절차

agent-browser로 웹에서 추출한 이미지를 기존 노트에 붙이는 경우:

1. **agent-browser로 이미지 다운로드**: URL에서 이미지를 `/workspace/media/<filename>` 경로에 저장한다.
2. **update-note + attachments로 노트 업데이트**: `target_path`에 대상 노트 경로를 지정하고, `attachments`에 다운로드한 이미지 경로를 포함한다. `content`에는 이미지와 함께 추가할 텍스트(설명, 출처 URL 등)를 넣는다.
...
```json
{
  ...
  "target_path": "notes/my-note.md",
  "operation": "append",
  "content": "출처: https://example.com/article",
  "attachments": [
    {
      "source_path": "/workspace/media/downloaded-image.jpg",
      "filename": "downloaded-image.jpg",
      "dest_filename": "ref-descriptive-name.jpg"
    }
  ]
}
```

`content`에 이미지 출처 URL이나 보조 설명을 포함하면 vault-watcher가 텍스트 먼저 append 후 `![[dest_filename]]`을 자동 삽입한다.
