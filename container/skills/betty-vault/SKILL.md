...
메시지에 `[Photo: ...]` 태그가 N개 있으면 `attachments` 배열도 반드시 N개여야 한다.
JSON을 생성하기 전에 메시지 내 `[Photo:]` 태그 수를 세고, 그 수와 attachments 배열
길이가 일치하는지 확인한다. 불일치 시 누락된 `[Photo:]` 태그의 경로를 찾아 추가한다.
...
agent-browser로 URL에서 추출한 이미지는 `/workspace/media-staging/<filename>` 경로에 저장한다.
attachments 항목의 source_path는 `/workspace/media-staging/<filename>` 형식을 사용한다.
`/workspace/media/`(읽기전용)와 달리 `/workspace/media-staging/`는 쓰기 가능하다.

### 대상 미디어 유형

- 포함: photo(jpg/jpeg/png/gif/webp), animation(mp4), document(이미지 확장자)
- 제외: video, voice, audio 등 비이미지 미디어
